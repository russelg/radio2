...
class ThemeChooser extends React.Component {
  constructor(props) {
    super(props)
    this.state = {
      selected: localStorage.hasOwnProperty('css') ? localStorage['css'] : document.querySelector('#change_stylesheet').href
    }
  ...
  render() {
    return (
      <select id='theme_chooser' value={this.state.selected} onChange={this.onChange}>
